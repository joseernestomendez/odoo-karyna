...
class procurement_order(osv.Model):
    ...
    _columns = {
         'from_merged': fields.boolean('Merged'),
    ...
    def do_merge(self, cr, uid, ids, context={}):
        ...
        def make_key(br, fields):
            ...
            for field in fields:
                ...
                if field in ('product_id', 'location_id', 'rule_id'):
                    if not field_val:
                        field_val = False
        ...
        for procurement in self.browse(cr, uid, ids):
            if procurement.state == 'draft' or procurement.state == 'running':
                order_key = make_key(procurement, (
                    'product_id', 'location_id', 'rule_id'))
        ...
        for order_key, (order_data, old_ids) in new_orders.iteritems():
            ...
            self.pool.get('procurement.order').write(cr, uid, neworder_id, {'from_merged': True}, context=context)
            orders_info.update({neworder_id: old_ids})
            neworders.append(neworder_id)
            for old_id in old_ids:
                #self.cancel(cr, uid, old_id)
                # #self.unlink(cr, uid, old_id)
                # wf_service = netsvc.LocalService("workflow")
                # wf_service.trg_validate(
                #     uid, 'procurement.order', old_id, 'button_cancel', cr)
                self.pool.get('procurement.order').write(cr, uid, old_id, {'merged': True}, context=context)
                #self.cancel(cr, uid, old_id)
